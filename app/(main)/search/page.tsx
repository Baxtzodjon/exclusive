...
//                         {
//                             category_btns.map((category_btn, i) => {
//                                 return (
//                                     <button className="bg-[#FFFFFF] text-start max-w-full text-[#808080] rounded text-[15px] hover:bg-[#DB4444] hover:text-white px-2 py-1" key={i}>{category_btn.name}</button>
//                                 );
//                             })
//                         }
...
//                 <h2 className="text-[#000000] text-[18px] sm:text-[24px] font-bold leading-normal hidden lg:block">Товары по запросу: <span>"Видеокарта Palit RTX3080Ti GameRock 12GB"</span></h2>
...
const SearchPage = () => {
    ...
    const [maxPrice, setMaxPrice] = useState<number>(8405); // Задаём максимальную цену
    ...
    useEffect(() => {
        if (selectedCategory === '') {
            setFilteredProducts(products); // Если категория не выбрана, показываем все продукты
        } else {
            const filtered = products.filter((product) => product.catg_prod.toLowerCase() === selectedCategory.toLowerCase());
            setFilteredProducts(filtered); // Фильтруем по категории
            setShowTitle(selectedCategory) /* checked */
        }
    ...
    const handleRangeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Изменяем фоновый цвет ползунка на основе значения
        const slider = e.target as HTMLInputElement;
        const percentage = (value / 8405) * 100; // Преобразуем в проценты
    ...
    useEffect(() => {
        const queryParams = new URLSearchParams(window.location.search); // Получаем параметры из URL
        const query = queryParams.get('query'); // Извлекаем параметр query

        // Если query изменяется, фильтруем продукты
        if (query) {
            ...
            // Если query нет, показываем все продукты
            setFilteredProducts(products);
        }
    }, [products]); // Мы следим только за products, query получаем из window.location
    ...
    return (
        <div className="flex flex-col gap-10">
            ...
            <div className="w-full max-w-[1230px] mx-auto py-4 px-6 flex gap-[50px] flex-col lg:flex-row"> {/* max-w-4xl mx-auto p-6 */}
                ...
                <div className="flex flex-col gap-5 mb-[40px]">
                    ...
                    {/* <h2 className="text-[#000000] text-[18px] sm:text-[24px] font-bold leading-normal hidden lg:block">{showTitle ? `Searching for: ${showTitle}` : "Browse Products"}</h2> */}

                    <h2 className="text-[#000000] text-[18px] sm:text-[24px] font-bold leading-normal hidden lg:block">
                        {showTitle
                            ? `Searching for: ${showTitle}` // Если есть запрос поиска, показываем текст поиска
                            : selectedCategory
                                ? `Showing products for category: ${selectedCategory}` // Если выбрана категория, показываем её
                                : 'Browse Products'} {/* Если ничего не выбрано, показываем "Browse Products" */}
                    </h2>

                    {/* <h2>{selectedCategory ? `Showing products for category: ${selectedCategory}` : 'Select a category'}</h2> */}
